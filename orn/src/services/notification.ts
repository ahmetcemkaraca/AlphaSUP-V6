...
// Lightweight local SMS placeholders (Phase 7 stubs to avoid shared package dependency)
type SMSConfig = {
...
class SMSTemplateService {
  render(templateKey: string, variables: Record<string, any>): string {
...
class NotificationManager {
  constructor(
    ...
    private templates: SMSTemplateService
  ) {}
...
export class NotificationService {
   ...
   * Phase 7: Initialize SMS services
   */
  private initializeSMSServices(): void {
    console.log(
      '🚧 [Notification Service] SMS servislerini başlatıyor - Phase 7 Implementation'
    );
    ...
    try {
      ...
      if (customer.email) {
        await this.sendEmailNotification(
          customer.email,
          'Ödeme Makbuzu - AlphaSUP',
          this.generatePaymentReceiptEmail(payment, customer, bookingId)
        );
      }
  ...
  async sendPaymentFailedNotification(
    ...
    try {
      ...
      if (customer.email) {
        await this.sendEmailNotification(
          customer.email,
          'Ödeme Başarısız - AlphaSUP',
          this.generatePaymentFailedEmail(customer, bookingId, failureReason)
        );
      }
      ...
      console.log(
        `Payment failed notification sent to customer: ${customerId}`
      );
    } catch (error) {
  ...
  async sendRefundNotification(
    ...
    try {
      ...
      if (customer.email) {
        await this.sendEmailNotification(
          customer.email,
          'İade İşlemi - AlphaSUP',
          this.generateRefundEmail(customer, bookingId, amount)
        );
      }
  ...
  async sendSMSNotification(
    ...
    console.log(
      '🚧 [Notification Service] SMS bildirimi gönderiliyor - Phase 7 Implementation',
      {
        type,
   ...
   */
  async getCustomerSMSPreferences(customerId: string): Promise<{
    smsEnabled: boolean;
    ...
    console.log(
      '🚧 [Notification Service] SMS tercihleri getiriliyor - Phase 7 Implementation',
      { customerId }
    );
   ...
   */
  async updateCustomerSMSPreferences(
    customerId: string,
    preferences: {
      smsEnabled?: boolean;
      bookingConfirmations?: boolean;
    ...
    console.log(
      '🚧 [Notification Service] SMS tercihleri güncelleniyor - Phase 7 Implementation',
      {
        customerId,
        preferences,
      }
   ...
   */
  private formatPaymentMethod(paymentMethod: Payment['paymentMethod']): string {
    switch (paymentMethod.type) {
      case 'card':
      default:
        return `${paymentMethod.card?.brand?.toUpperCase() || 'CARD'} **** ${
   ...
   */
  private async saveNotificationLog(log: {
    type: NotificationType;
    recipient: string;
    subject?: string;
    content: string;
