...
const paymentService = new payment_1.default();
 ...
 */
router.post('/create-intent', authMiddleware.authenticate, async (req, res) => {
    try {
        const { bookingId, amount, depositOnly, depositPercentage, currency, savePaymentMethod, } = req.body || {};
        ...
        const { paymentIntent, clientSecret } = await paymentService.createPaymentIntent(bookingId, req.user.uid, amt, {
            depositOnly,
 ...
 * Get payment details
 */
router.get('/:paymentId', authMiddleware.authenticate, async (req, res) => {
    try {
 ...
 */
router.get('/booking/:bookingId', authMiddleware.authenticate, async (req, res) => {
    try {
 ...
 */
router.post('/:paymentId/refund', authMiddleware.authenticate, authMiddleware.requireAdmin, async (req, res) => {
    try {
        const { paymentId } = req.params;
        ...
        const refund = await paymentService.createRefund(paymentId, amt, reason, req.user.uid, {
            adminNotes,
        });
        res.status(200).json({
            success: true,
