...
// GET / - list services
router.get('/', auth.authenticate, auth.requireEditor, async (req, res) => {
    try {
...
router.get('/:id', auth.authenticate, auth.requireEditor, async (req, res) => {
    try {
...
// POST / - create service
router.post('/', auth.authenticate, auth.requireEditor, async (req, res) => {
    try {
        ...
        for (const key of required) {
            if (payload[key] === undefined ||
                (key === 'name' && String(payload[key]).trim() === '')) {
                return badRequest(res, `Missing field: ${key}`);
            }
        }
        ...
        const docRef = await db.collection('services').add({
            ...payload,
            bookingCount: 0,
            displayOrder: typeof payload.displayOrder === 'number'
...
router.put('/:id', auth.authenticate, auth.requireEditor, async (req, res) => {
    try {
        const id = String(req.params.id || '').trim();
        if (!id)
            return badRequest(res, 'Missing id');
...
router.patch('/:id/toggle', auth.authenticate, auth.requireEditor, async (req, res) => {
    try {
        const id = String(req.params.id || '').trim();
        if (!id)
            return badRequest(res, 'Missing id');
