...
router.get('/profile/:userId', authMiddleware.authenticate, async (req, res) => {
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(customerData, 'stripeCustomerId')) {
            delete customerData.stripeCustomerId;
        }
        res.status(200).json({
            success: true,
            data: {
...
router.post('/', authMiddleware.authenticate, async (req, res) => {
    try {
        ...
        const { firstName, lastName, phoneNumber, email, dateOfBirth, emergencyContact, swimmingLevel, preferences, } = req.body;
...
router.put('/profile/:userId', authMiddleware.authenticate, async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        const { firstName, lastName, phoneNumber, dateOfBirth, emergencyContact, swimmingLevel, preferences, } = req.body;
        const updateData = {
            updatedAt: firebase_admin_1.default.firestore.FieldValue.serverTimestamp(),
        };
        if (firstName)
        ...
        if (swimmingLevel)
            updateData.swimmingLevel = swimmingLevel;
        if (preferences)
...
router.get('/bookings/:userId', authMiddleware.authenticate, async (req, res) => {
    try {
        const { userId } = req.params;
